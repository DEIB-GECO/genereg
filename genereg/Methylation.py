...
def extract_methylation(tumor, platform, gencode_version):
	...
	methyl_areas_reg = coding_transcripts.reg_project(['gene_id','gene_name','entrez_gene_id'], new_field_dict = {'start': coding_transcripts.start - 4000, 'stop': coding_transcripts.start + 1000})
	...
	# The dictionary has the Gene Symbols of the genes of interest as keys and a dataframe containing all the methylation regions with genomic coordinates that are within +4000/-1000 bases from the TSS, for each gene of interest.
	...
	with open ('./3_TCGA_Data/Common Aliquots.txt', 'w') as fp:
	...
	aliquot_file = open('./3_TCGA_Data/Common Aliquots.txt', 'r')
	...
	writer = ExcelWriter('./3_TCGA_Data/Methylation/Methylation Values.xlsx')
