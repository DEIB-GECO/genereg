...
def linear_regression(pathway, n_data_matrix):

	"""
	The LINEAR_REGRESSION operation executes the linear regression analysis per pathway and per data matrix, considering as inputs of the model only the features selected during the previous feature selection procedure. Results are exported locally either in Excel or text files.
	...
	for current_gene in SYMs_current_pathway:
		...
		text_file = open('./5_Data_Analysis/'+pathway+'/FeatureSelection/M'+model+'/Features - Gene '+gene_ID+' ['+current_gene+'].txt', 'r')
		...
		if len(features_sel) > 0:
			...
			with open ('./5_Data_Analysis/'+pathway+'/LinearRegression/M'+model+'/LinReg Summary - Gene '+gene_ID+' ['+current_gene+'].txt', 'w') as fp:
			...
			coeff_path = './5_Data_Analysis/'+pathway+'/LinearRegression/M'+model+'/Coefficients/'
			writer = ExcelWriter(coeff_path+'Coefficients (M'+model+') - Gene '+gene_ID+' ['+current_gene+'].xlsx')
			...
			ci_path = './5_Data_Analysis/'+pathway+'/LinearRegression/M'+model+'/Confidence Intervals/'
			writer = ExcelWriter(ci_path+'Confidence Intervals (M'+model+') - Gene '+gene_ID+' ['+current_gene+'].xlsx')
			...
			corr_path = './5_Data_Analysis/'+pathway+'/LinearRegression/M'+model+'/Correlation Matrix/'
			writer = ExcelWriter(corr_path+'Correlation Matrix (M'+model+') - Gene '+gene_ID+' ['+current_gene+'].xlsx')
	...
	writer = ExcelWriter('./5_Data_Analysis/'+pathway+'/LinearRegression/M'+model+'/Linear Regression R2 SCORES.xlsx')
