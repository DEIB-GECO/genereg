...
def linear_regression(gene_set, n_data_matrix):
	...
	for current_gene in SYMs_current_pathway:
		...
		text_file = open('./5_Data_Analysis/'+gene_set+'/FeatureSelection/M'+model+'/Features - Gene '+gene_ID+' ['+current_gene+'].txt', 'r')
		...
		if len(features_sel) > 0:
			...
			with open ('./5_Data_Analysis/'+gene_set+'/LinearRegression/M'+model+'/LinReg Summary - Gene '+gene_ID+' ['+current_gene+'].txt', 'w') as fp:
	...
	writer = ExcelWriter('./5_Data_Analysis/'+gene_set+'/LinearRegression/M'+model+'/Linear Regression R2 SCORES.xlsx')
