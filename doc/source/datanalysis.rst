...
and
	...
	* a **REGRESSION** process that fits a linear model on each individual target gene and its data matrixes, starting from the set of selected features as inputs and normalizing (i.e. *Z-score normalization*) all the data.

Here is an example of the linear regression process for the sample gene TKT and the selection of its relevant features:

.. image:: images/lreg.png

|

Following this procedure, it's possible to introduce an alteration in the results due to the order in which features are included in the models. In order to avoid this, the regression procedure is logically based only on the biological knowledge and it therefore analyzes the full M2 matrix (which in turn contains M1) at first, including all at once in the model all the features whose exchanged order could cause different results in the analysis. Subsequently, data analysis on M3 and M5 is performed, finally executing only three regression processes for each target gene. 
